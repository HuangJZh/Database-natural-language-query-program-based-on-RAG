...
async def init_test_system(background_tasks: BackgroundTasks):
    ...
    def _init_test():
        ...
        try:
            ...
            ts = database_rag_system.AdvancedRAGSystem(cfg, db_cfg, llm=shared_llm, embeddings=shared_emb, init_llm=False, init_embeddings=False)
            ts.load_rag_system()
            _test_system = ts
            _test_initialized = True
        except Exception:
...
async def run_tests():
    ...
    comparator=database_rag_system.EnhancedRAGComparison(DEFAULT_DB_CONFIG,_test_system.config if hasattr(_test_system,'config') else _test_system)
    try:
        comparator.connect()
        ...
        if mode=='chat':
            ...
            if _test_system:
                if shared_embeddings is not None:
                    ...
                    if getattr(_test_system,'vector_db',None) is None and os.path.exists(_test_system.config.VECTOR_DB_DIR):
                        _test_system.load_rag_system()
                else:
                    _test_system.ensure_embeddings()
            if _dialogue_system and getattr(_dialogue_system,'rag_system',None):
...
def _serialize_response(resp: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        for d in docs:
            ...
            simple_docs.append({
                'page_content': (content[:1000] + '...') if isinstance(content, str) and len(content) > 1000 else content,
                'metadata': metadata
            })
...
:None            }
            ...
            try:
                if isinstance(sql_no_rag,str) and sql_no_rag.lower().startswith('SELECT'):
            ...
            try:
                if isinstance(sql_with_rag,str) and sql_with_rag.lower().startswith('SELECT'):
